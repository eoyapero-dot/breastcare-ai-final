...
export function DashboardOverview() {
  // Using 'any' here temporarily so it catches both old and new database rows safely
  ...
  // Safely check for risk score (catches 'High Risk', 'High', etc.)
  const highRiskCount = patients.filter(p => 
    p.risk_score === "High Risk" || p.risk_level === "High" || String(p.risk_score).includes("High")
  ).length
  
  const lowRiskCount = patients.filter(p => 
    p.risk_score === "Low Risk" || p.risk_level === "Low" || String(p.risk_score).includes("Low")
  ).length
  
  // Calculate assessments done in the last 24 hours safely
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Latest AI patient risk evaluations
              </p>
            </div>
            ...
            <Table>
              ...
              <TableBody>
                {patients.slice(0, 5).map((patient, index) => {
                  ...
                  // These check for your new MySQL columns, but fallback safely if they are missing
                  const pName = patient.patient_name || patient.full_name || "Unknown Patient";
                  const pId = patient.patient_id || patient.id || `00${index + 1}`;
                  ...
                  const pDate = patient.created_at || patient.prediction_date;
                  ...
                  return (
                    ...
                    >
                      <TableCell>
                        <div>
                          <div className="font-medium text-foreground">{pName}</div>
                          <div className="text-xs text-muted-foreground">
                            PT-{pId} · {pAge ? `${pAge} yrs` : "Age N/A"}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-right text-sm text-muted-foreground">
                        {pDate ? new Date(pDate).toLocaleDateString() : "Just Now"}
                      </TableCell>
                    </TableRow>
                  )
                })}
        ...
        <Card className="border-border/50 bg-card/50 shadow-lg shadow-black/5">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground">
              Patient population by prediction
            </p>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="mt-6 rounded-lg bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900 p-4">
              <div className="flex items-center gap-2 text-sm">
                <AlertTriangle className="h-4 w-4 text-red-600" />
                <span className="font-bold text-red-700 dark:text-red-400">Action Required</span>
              </div>
